...
<div class="container pb-6 pt-6">
    ...
    <div class="columns">
        <div class="column is-one-third">
            ...
            if ($categorias->rowCount() > 0) {
                ...
                foreach ($categorias as $row) {
                    echo '<a href="index.php?vista=product_category&category_id=' . $row['categoria_id'] . '" class="button is-link is-inverted is-fullwidth">' . $row['nombre'] . '</a>';
                }
            } else {
        ...
        <div class="column">
            ...
            $check_categoria = $check_categoria->query("SELECT * FROM categorias WHERE categoria_id='$categoria_id'");

            if ($check_categoria->rowCount() > 0) {

                $check_categoria = $check_categoria->fetch(PDO::FETCH_ASSOC);

                echo '
                        <h2 class="title has-text-centered">' . $check_categoria['nombre'] . '</h2>
                        <p class="has-text-centered pb-6" >' . $check_categoria['ubicacion'] . '</p>
                    ';
